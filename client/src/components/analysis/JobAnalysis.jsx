...
function JobAnalysis({ applicationId, jobDescription }) {
  ...
  useEffect(() => {
    const loadAnalysis = async () => {
      try {
        const analysis = await getJobAnalysis(applicationId);
        ...
        setError(
          error.response?.data?.message || "Failed to load job analysis.",
        );
      }
    };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setError(
        error.response?.data?.message || "Failed to analyze job description.",
      );
    } finally {
      setAnalyzing(false);
    }
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {jobAnalysis.keywords?.length > 0 ? (
                ...
                <p className="text-sm text-[#9A9F97]">No keywords found.</p>
              )}
            </div>
          </div>
