...
const StudyPlan = ({ applicationId }) => {
  ...
  const loadStudyPlan = async () => {
    try {
      ...
      if (data) {
        setStudyPlan(normalizeTasks(data));
      }
    } catch (error) {
      setError(error.response?.data?.message || "Failed to load study plan.");
    }
  };
  ...
  const handleTaskToggle = async (stepOrder, taskIndex, completed) => {
    try {
      setError("");

      const updatedPlan = await updateStudyTask(
        applicationId,
        stepOrder,
        taskIndex,
        completed,
      ...
      setStudyPlan(normalizeTasks(updatedPlan));
    } catch (error) {
      setError(error.response?.data?.message || "Failed to update study task.");
    }
  };

  const allTasks = studyPlan?.steps?.flatMap((step) => step.tasks || []) || [];
    ...
    <div className="rounded-[24px] bg-white p-6 shadow-[0_12px_40px_rgba(23,36,58,0.04)] sm:p-7">
      ...
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#667085]">
            A focused preparation plan based on the skills you need to improve
            for this role.
          </p>
        </div>
      ...
      {!studyPlan && !loading && !error && (
        <div className="mt-6 rounded-2xl border border-[#DCDDD7] bg-[#F5F3EE] p-6">
          ...
          <p className="mt-1 text-sm leading-6 text-[#667085]">
            Create a personalized preparation roadmap based on your skill gaps
            for this role.
          </p>
        </div>
      )}
      ...
      {studyPlan && (
        <div className="mt-7">
          ...
          {totalTasks > 0 && (
            <div className="mt-6 rounded-2xl border border-[#DCDDD7] p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-[#17243A]">
                    {completedTasks} of {totalTasks} tasks completed
                  </p>
                </div>

                <p className="text-lg font-bold text-[#17243A]">{progress}%</p>
              </div>

              <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#E8E8E3]">
                <div
                  className="h-full rounded-full bg-[#8BAE42] transition-all duration-300"
                ...
                {studyPlan.steps.map((step, index) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      {index !== studyPlan.steps.length - 1 && (
                        ...
                        <div className="mt-5 border-t border-[#E8E8E3] pt-5">
                          ...
                          <div className="mt-3 space-y-3">
                            {step.tasks.map((task, taskIndex) => (
                              <label
                                key={taskIndex}
                                className="flex cursor-pointer items-start gap-3 rounded-xl p-2 -mx-2 transition hover:bg-[#F5F3EE]"
                              >
                                <input
                                  type="checkbox"
                                  ...
                                  className="mt-1 h-4 w-4 shrink-0 cursor-pointer accent-[#8BAE42]"
                                />

                                <p
                                  className={`text-sm leading-6 transition ${
                                    task.completed
                                      ? "text-[#9A9F97] line-through"
                                      : "text-[#667085]"
                                  }`}
                                >
                                  {task.text}
          ...
          {(!studyPlan.steps || studyPlan.steps.length === 0) && (
            <div className="mt-6 rounded-2xl border border-[#DCDDD7] bg-[#F5F3EE] p-6">
              <p className="text-sm font-semibold text-[#17243A]">
              ...
              <p className="mt-1 text-sm leading-6 text-[#667085]">
                Regenerate the plan to create a preparation roadmap.
              </p>
            </div>
          )}
        </div>
